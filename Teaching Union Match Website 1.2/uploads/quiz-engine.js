...
window.QUIZ = (function () {
    ...
    // Q2b — personal participation stance
    {
      ...
      options: [
        ...
        { id: "non_member",    label: "I'd join strikes even if I'm not in the union that called them",
          desc: "I'd want support and protection from an organisation I trust, but I'd still stand with colleagues when they strike.",
          w: { collective: 2, apolitical: 1, protection: 1 } },
        { id: "cross",         label: "Stay in the classroom — I'd cross the picket line",
          desc: "I wouldn't want to participate in a strike, even one called by my own union.",
  ...
  function score({ role, nation, answers }) {
    ...
    return { ranked: rawScores, maxScore: 100, reasons, dynAfford, tensions };
  }
  ...
  function personalisedCost({ org, role, nation, answers, fte }) {
    ...
    if (roleData.ftRange && fteSafe >= 1.0) {
      ...
      const noteFor = {
        unite: "based on Low Pay rate (≤£26k gross)",
