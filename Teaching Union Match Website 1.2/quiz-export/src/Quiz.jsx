...
// ── Result card ────────────────────────────────────────────────────────────
function ResultCard({ entry, reasons, rank, isTop, defaultOpen }) {
    ...
    <article className="card" style={{ overflow: "hidden", borderColor: isTop ? "var(--c-primary)" : "var(--c-border)" }}>
      ...
      {open && (
        <div style={{ padding: "4px 22px 22px", borderTop: "1px solid var(--c-border)", background: "var(--c-surface)" }}>
          ...
          {/* Why this fits — concise, no formula exposition */}
          ...
          <div style={{ marginTop: 22, padding: "16px 18px", background: "var(--c-background-2)", borderRadius: 12 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {Object.entries(o.costs).map(([k, v]) => (
...
function Quiz() {
  ...
  // Restore from URL hash (for share link) or localStorage
  useEffect(() => {
    try {
  ...
  useEffect(() => {
    try { localStorage.setItem("wtu_quiz", JSON.stringify({ stage, answers, qIndex })); } catch {}
  }, [stage, answers, qIndex]);

  // Question list: nation → role → core 5.
  const questions = useMemo(() => [
    window.QUIZ.NATION_QUESTION,
    window.QUIZ.ROLE_QUESTION,
    ...window.QUIZ.CORE_QUESTIONS,
  ], []);
  ...
  // Share link (encodes answers into URL hash — no server)
      ...
      <div ref={scrollRef} style={{ maxWidth: 720, margin: "0 auto" }}>
        <div className="card" style={{ padding: 40 }}>
          ...
          <p style={{ fontSize: 17, color: "var(--c-text-muted)", marginBottom: 28, lineHeight: 1.6 }}>
            Seven quick questions, around two minutes. No sign-up, no email, no tracking. Your answers stay in your browser.
          </p>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14, marginBottom: 32 }} className="intro-stats">
            {[
              { n: "17", l: "organisations compared" },
    ...
    return (
      <div ref={scrollRef} style={{ maxWidth: 680, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 14 }}>
          ...
          {answers.nation && qIndex > 0 && (
            <span className="tag">{window.NATIONS[answers.nation]?.label}{answers.role ? ` · ${window.ROLES[answers.role]?.label}` : ""}</span>
          )}
        </div>
    ...
    <div ref={scrollRef} style={{ maxWidth: 860, margin: "0 auto" }}>
      ...
      <aside className="card" style={{ padding: 28, marginTop: 32, background: "var(--c-background-2)" }}>
        ...
        <ol style={{ paddingLeft: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 10 }}>
          {[
            "Have colleagues mentioned their experience of a union or support provider? Local knowledge matters.",
            "Does your school already have active reps for any of these organisations? Having a rep in the building is a real advantage.",
            "Are you likely to move into or out of a leadership role soon? Eligibility can shift.",
            "Most organisations will have a quick conversation with you for free — it's a good way to sanity-check a shortlist.",
          ].map((q, i) => (
