...
(function () {
  ...
  window.WTU_LAYOUT = {
    mount(active) {
      ...
      // Skip-link target: tag the first content element after the header so
      // keyboard users can jump past the nav on every page. NEVER overwrite an
      // existing id (e.g. #heroA on the homepage) — reuse it and point the
      // skip link at it instead.
      if (!document.getElementById('main-content')) {
        const hdr = document.querySelector('.site-header');
        const target = document.querySelector('main') || (hdr && hdr.nextElementSibling);
        if (target) {
