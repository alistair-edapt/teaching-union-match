...
// ── Subtle demographic proxy ────────────────────────────────────────────────
// We never ask for age or gender. Instead we derive a *coarse* career stage
// (and a loose age band) from answers the quiz already collects — role and,
// for teachers, years qualified. This is an approximation for aggregate
// profiling only; it is not a real age and is never tied to an individual.
function wtuDeriveProfile(answers) {
  const role = answers.role;
  const yq = answers.yearsQualified;
  if (role === "trainee") return { career_stage: "trainee", age_band: "21-25" };
  if (role === "leader")  return { career_stage: "leadership", age_band: "35+" };
  if (role === "support") return { career_stage: "support_staff", age_band: null };
  if (role === "supply")  return { career_stage: "supply", age_band: null };
  if (role === "teacher") {
    if (yq === "ect1" || yq === "ect2") return { career_stage: "early_career", age_band: "22-27" };
    if (yq === "ect3")                  return { career_stage: "developing", age_band: "24-30" };
    return { career_stage: "experienced_teacher", age_band: "28+" };
  }
  return { career_stage: "unknown", age_band: null };
}

// ── Balanced results summary ────────────────────────────────────────────────
// A short, plain-English framing of the ranked results: why the top option
// leads, the main trade-off + the close alternative, and a neutrality note.
// Built from the engine's own reason/anti-reason data so it always matches the
// scoring. Kept deliberately high-level (max two priorities) so it frames the
// results without re-explaining every card.
function ResultsSummary({ results }) {
  const ranked = (results && results.ranked) || [];
  if (!ranked.length) return null;
  const AX = window.QUIZ.AXIS_LABELS || {};
  const L = (a) => (AX[a] || a).toLowerCase();
  const joinAnd = (arr) =>
    arr.length <= 1 ? (arr[0] || "") : `${arr.slice(0, -1).join(", ")} and ${arr[arr.length - 1]}`;

  const top = ranked[0];
  const runner = ranked[1];
  const topReasons = (results.reasons?.[top.key] || []).map(r => r.axis);
  const topLead = topReasons.slice(0, 2).map(L);
  const topGap = (results.antiReasons?.[top.key] || [])[0];

  let altAxis = null;
  if (runner) {
    const rReasons = (results.reasons?.[runner.key] || []).map(r => r.axis);
    altAxis = rReasons.find(a => !topReasons.includes(a)) || rReasons[0] || null;
  }

  return (
    <aside className="card" style={{ padding: 28, marginBottom: 30, borderLeft: "4px solid var(--c-primary)" }}>
      <div style={{ fontSize: 12, letterSpacing: ".12em", textTransform: "uppercase", fontWeight: 700, color: "var(--c-primary)", marginBottom: 12 }}>
        In short
      </div>
      <p style={{ fontSize: 16, lineHeight: 1.7, color: "var(--c-text)", marginBottom: 14 }}>
        <b>{top.org.short}</b> sits at the top {topLead.length
          ? <>because your answers leaned toward <b>{joinAnd(topLead)}</b> — areas where it scores well</>
          : <>because it best fits the overall balance of your answers</>}.
      </p>
      {runner && (
        <p style={{ fontSize: 16, lineHeight: 1.7, color: "var(--c-text-muted)", marginBottom: 14 }}>
          <b>{runner.org.short}</b> is also a close match to these priorities{altAxis ? <>, leaning a little more into <b>{L(altAxis)}</b></> : null} — worth a look if that feels important to you.
        </p>
      )}
      <p style={{ fontSize: 14.5, lineHeight: 1.7, color: "var(--c-text-muted)", margin: 0 }}>
        Every option here is scored by the same engine against the priorities you gave. Lower-ranked options aren't necessarily worse; they're matched to different priorities that appear less important to you based on your answers. Treat this as an informed starting point, then read the detail and trust your own judgement.
      </p>
    </aside>
  );
}
...
// ── Seeded shuffle ────────────────────────────────────────────────────────
// Produces a deterministic permutation of an array given a seed string.
// Stable within a session (so back/forward doesn't reorder options), but
...
function ResultCard({ entry, reasons, antiReasons, rank, isTop, defaultOpen, personalCost, userRole }) {
  ...
  return (
    <article className="card" style={{
      overflow: "hidden",
      border: isTop ? "2px solid var(--c-primary)" : "1px solid var(--c-border)",
      boxShadow: isTop ? "0 10px 30px -12px rgba(61, 32, 73, .25)" : undefined,
    }}>
      <header
        onClick={() => {
          const next = !open;
          setOpen(next);
          if (next) window.WTU_ANALYTICS?.event('result_expand', {
            org_slug: entry.key, rank: rank + 1, tier: tier.label,
          });
        }}
        style={{
        ...
        <div style={{ padding: "4px 22px 22px", borderTop: "1px solid var(--c-border)", background: "var(--c-surface)" }}>
          ...
          <div style={{ marginTop: 20, display: "flex", gap: 10, alignItems: "center", justifyContent: "flex-end", flexWrap: "wrap" }}>
            <a href={`https://www.${o.website}`} target="_blank" rel="noopener noreferrer" className="btn btn-secondary" data-outbound={entry.key}
              onClick={() => window.WTU_ANALYTICS?.event('result_link_click', { org_slug: entry.key, rank: rank + 1, tier: tier.label, link_type: 'website' })}>
              Visit {o.website}
            ...
            <a href={`https://www.${o.website}`} target="_blank" rel="noopener noreferrer" className="btn btn-primary" data-outbound={entry.key}
              onClick={() => window.WTU_ANALYTICS?.event('result_link_click', { org_slug: entry.key, rank: rank + 1, tier: tier.label, link_type: 'join' })}>
              See joining options
            </a>
          </div>
        </div>
...
function Quiz() {
  ...
  // One random seed per Quiz mount — used to shuffle option order for
  // randomize:true questions. Stable within a session via useRef.
  const sessionSeed = useRef((Math.random() * 0x7fffffff | 0).toString(36));
  ...
  // For questions flagged randomize:true, shuffle options once per question
  // per session. Any option with id==="none" is kept anchored at the end.
  const currentQDisplay = useMemo(() => {
    if (!currentQ?.randomize || !currentQ?.options) return currentQ;
    const opts = currentQ.options;
    const seed = sessionSeed.current + currentQ.id;
    // Options with a numeric anchorIndex stay locked to that slot; the rest are
    // shuffled and poured into the remaining slots in order. Options flagged
    // anchor:true (or a trailing "none") are pinned to the bottom.
    const hasFixedIndex = opts.some((o) => typeof o.anchorIndex === 'number');
    if (hasFixedIndex) {
      const fixed = opts.filter((o) => typeof o.anchorIndex === 'number');
      const shuffled = seededShuffle(opts.filter((o) => typeof o.anchorIndex !== 'number'), seed);
      const out = new Array(opts.length);
      fixed.forEach((o) => { out[o.anchorIndex] = o; });
      let k = 0;
      for (let i = 0; i < out.length; i++) if (!out[i]) out[i] = shuffled[k++];
      return { ...currentQ, options: out };
    }
    const isAnchored = (o) => o.anchor === true || o.id === 'none';
    const anchored = opts.filter(isAnchored);
    const shufflable = opts.filter((o) => !isAnchored(o));
    return { ...currentQ, options: [...seededShuffle(shufflable, seed), ...anchored] };
  ...
  useEffect(() => {
    if (stage === "results" && results?.ranked?.length) {
      ...
      const profile = wtuDeriveProfile(answers);
      ...
      // Comprehensive event — all individual answers in one GA4 event so you
      // can see a complete submission without correlating multiple quiz_answer
      // events. Visible in GA4 → Reports → Engagement → Events → quiz_complete_full.
      window.WTU_ANALYTICS?.event('quiz_complete_full', {
        a_role:     answers.role              || '',
        a_nation:   answers.nation            || '',
        a_years:    answers.yearsQualified    || '',
        a_priority: answers.priority          || '',
        a_strike_r: answers.strike_rights     || '',
        a_strike_p: answers.strike_participation || '',
        a_voice:    answers.voice             || '',
        a_support:  answers.support_type      || '',
        a_edu:      answers.edu_focus         || '',
        a_benefits: Array.isArray(answers.benefits)
                      ? answers.benefits.join(',')
                      : (answers.benefits || ''),
        a_involve:  answers.involvement       || '',
        a_pol_fund: answers.political_fund    || '',
        a_cost:     answers.cost              || '',
        top_1:      results.ranked[0]?.key    || '',
        top_1_tier: results.ranked[0]
                      ? window.QUIZ.tier(results.ranked[0].pct).label
                      : '',
        top_2:      results.ranked[1]?.key    || '',
        top_3:      results.ranked[2]?.key    || '',
      });
  ...
  const shareLink = () => {
    ...
    window.WTU_ANALYTICS?.event?.("copy_share_link", { org_slug: results?.ranked?.[0]?.key || "" });
  ...
  // "Share my result" — a human-readable headline message rather than a raw
  // link. Uses the native share sheet on mobile; falls back to copying the
  // message text on desktop. Links to the quiz START (not this person's
  // answers) so recipients take their own quiz.
  const [shareResultText, setShareResultText] = useState("Share my result");
  const shareResult = () => {
    const top = results?.ranked?.[0];
    if (!top) return;
    const url = "https://teachingunionmatch.org.uk/quiz.html";
    const text = `I took the Teaching Union Match quiz and my strongest match was ${top.org.short}. If you want to find out your match, try it here: ${url}`;
    window.WTU_ANALYTICS?.event?.("share_result", { org_slug: top.key });
    if (navigator.share) {
      navigator.share({ text }).catch(() => {});
    } else {
      navigator.clipboard?.writeText(text).then(() => {
        setShareResultText("Message copied ✓");
        setTimeout(() => setShareResultText("Share my result"), 2200);
      });
    }
  };
      ...
      <div ref={scrollRef} style={{ maxWidth: 720, margin: "0 auto" }}>
        <div className="card" style={{ padding: 40 }}>
          ...
          <p style={{ fontSize: 17, color: "var(--c-text-muted)", marginBottom: 28, lineHeight: 1.6 }}>
            A short set of questions, around two minutes. No sign-up, no email capture — all completed anonymously.
          </p>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14, marginBottom: 32 }} className="intro-stats">
            {[
              ...
              { n: "0",   l: "accounts or emails needed" },
            ].map((s, i) => (
  ...
  if (stage === "quiz") {
    return (
      <div ref={scrollRef} style={{ maxWidth: 680, margin: "0 auto" }}>
        ...
        <div className="card q-step" style={{ padding: 36 }} key={currentQ.id}>
          <h2 style={{ fontSize: "clamp(24px, 2.6vw, 32px)", marginBottom: 10, lineHeight: 1.2 }}>{currentQDisplay.text}</h2>
          {currentQDisplay.subtitle && <p style={{ fontSize: 15, color: "var(--c-text-muted)", marginBottom: 24, lineHeight: 1.55 }}>{currentQDisplay.subtitle}</p>}
          {currentQDisplay.type === "slider" ? (
            <SliderInput
              question={currentQDisplay}
              value={answers[currentQ.id] ?? currentQDisplay.default}
              onChange={(v) => setAnswer(currentQ.id, v)}
              onContinue={handleSliderContinue}
            />
          ) : currentQDisplay.type === "multi" ? (
            <MultiSelectInput
              question={currentQDisplay}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {currentQDisplay.options.map((o, i) => (
  ...
  return (
    <div ref={scrollRef} style={{ maxWidth: 860, margin: "0 auto" }}>
      ...
      {/* Balanced framing of the ranked list — built from real reason data */}
      {results && results.ranked.length > 0 && <ResultsSummary results={results} />}

      {/* Controls: share / restart */}
      <div style={{ display: "flex", gap: 10, justifyContent: "center", marginBottom: 28, flexWrap: "wrap" }}>
        <button onClick={shareResult} className="btn btn-secondary">
          <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M7 1v8M4 4l3-3 3 3M2 9v3h10V9" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
          {shareResultText}
        </button>
